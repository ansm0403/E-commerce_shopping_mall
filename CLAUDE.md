...
- 셀러 **주문/배송** + 관리자 **주문 관리** — `(main)/seller/orders`(출고 대기 탭 기본 + 운송장 입력 모달)·`(admin)/admin/orders`(상태 필터 목록)·`[orderNumber]`(상세: 배송건별/전체 deliver) 실구현. `getSellerOrders`가 본인 shipments 도 실어줌(배송 처리 버튼 판단). **§7-5 해소**: `orderItem.sellerId ?? 0` → null(컬럼 nullable) + `order.paid` 리스너·정산 집계 null 스킵 — 0이면 shipments FK 위반으로 리스너 재시도 루프였다. 배송 왕복 e2e(출고→배송완료→구매확정→**정산 PENDING 자동 생성** 단언) 포함. 상세 로드맵 §1-A③·§2-A③.
- **정산** — 셀러 `(main)/seller/settlements`(요약 카드+내역)·관리자 `(admin)/admin/settlements`(pending 탭 기본 + 확정/지급 2단계 전이) 실구현. 정산 생성은 **이미 있던** `SettlementEventListener`(구매확정 `order.completed` → 셀러별 PENDING, 수수료 10%, 멱등)가 담당 — "생성 코드 없음" 사전 감사는 모듈 내부 리스너를 놓친 오판이었다. 백엔드 보강: confirm/pay에 DemoAccountGuard, `SettlementResponseDto` BaseModel 상속 제거(직렬화 함정 — id/createdAt 누락 버그). 이중 prefix 버그는 이미 해소돼 있었음. e2e: 셀러 조회/요약 + 확정→지급 전이 규칙까지 고정. 상세 로드맵 §1-A④·§2-A④.
...
- **RN Ops Companion Phase 2 완료(2026-09-21, `4ead4ca`)**: 관측성 심화 + 보안 UX. **소스맵**(app.json org·project + Metro Debug ID → Gradle 이 debug 아닌 빌드에서만 업로드. `release` 를 적지 않아야 네이티브 기본값이 소스맵 릴리즈명과 일치한다) · **beforeSend**(같은 에러 60s 1건 + 실행당 20건 = 쿼터 방어, 이메일·전화·Bearer·JWT·푸시토큰 마스킹) · **태그**(`screen` 은 usePathname 이 아니라 **useSegments** — 실제 경로는 카디널리티가 터진다) · **Release Health**(`GET /v1/ops/release-health`, Sentry sessions 프록시. `project` 를 빼면 쇼핑몰 웹 세션이 섞인다. 정렬은 세션 수가 아니라 `+N` 내림차순. **`interval: '1d'` 필수** — 미지정·1h 면 세션이 적은 **새 릴리즈가 응답에서 조용히 빠진다**) · **생체 인증**(덮개 방식 — 라우트가 아니라 Stack 위에 겹쳐서 잠긴 동안 뒤에서 딥링크 이동이 끝난다). **DB 변경 0건.** `eas.json` preview 에 `autoIncrement` (RN 앱은 커밋 SHA 를 몰라 versionCode 가 릴리즈를 가르는 유일한 수단). 상세 `docs/learning/ops-companion/03-observability-and-biometrics.md`. 다음 = Phase 3(AI 분석).
- **RN Ops Companion Phase 3 완료(2026-09-21, main `89a02bc` = PR #33, 운영 배포·Sentry span 확인까지)**: `POST /v1/ops/incidents/:id/analysis`(`ops-analysis.service.ts`) — `getIncident` 재사용 → `scrubText` 프롬프트 → **`LlmClient.generate` 그대로 재사용** → `parseAnalysis`(관대하게 읽고 엄격하게 검증, 코드펜스·잡담 벗김) → 위반 시 사유를 실은 **교정 재시도 1회** → `ops_analyses`(마이그레이션 `OpsAnalyses1789968335669`, +`raw_text`). force 없는 요청은 **최근 행을 상태 무관하게**(parse_failed 포함) 반환 — 몰래 재시도로 쿼터를 태우지 않는다. 분당 상한 `OPS_ANALYSIS_MAX_PER_MIN`(기본 5)→429, 이슈별 락→409, `simulate:'parse_failed'` 는 NODE_ENV=production 에서 무시. 앱 S4 `incidents/analysis/[id]`(상세의 형제 경로) + `[DEV]` 강제 실패 버튼. Sentry span 은 앱 `tracesSampler` **이름 필터**(`ops.analysis*` 만 100%) + 백엔드 `ops.analysis.llm`. 상세 `docs/learning/ops-companion/04-ai-analysis.md`. 다음 = Phase 4(평가 루프). 인수인계 `docs/roadmap/_next-session-phase4.md`.
- **RN Ops Companion Phase 4 완료(2026-09-22, main `8610aca` = PR #35, 운영 배포·마이그레이션 1건 적용까지)**: 평가 루프. **실측: test 6건 블라인드 채점 v1 5/6(83%) vs v2 4/6(67%), 쌍둥이 제외 3/4 동률 — few-shot 이 승인율을 올리지 못했다(표본 6건, 단정 불가). v1·v2 모두 CORS 이슈에 같은 오답 → 보강 후보 1번(소스 코드 읽기)의 근거.** 상세 `docs/learning/ops-companion/05-review-loop.md`. **`ops_reviews`**(UNIQUE(analysis_id, reviewer_id), 마이그레이션 `OpsReviews1790001959888` — `ops_analyses` 에 `incident_title`·`exception_text`·`few_shot_ids` 도 추가, 로컬 적용·운영 미적용) + `ops-review.service.ts`: `GET /v1/ops/analyses/pending`(**블라인드 — promptVersion 을 응답에서 뺌**, `md5(id:reviewerId)` 고정 셔플) · `POST /v1/ops/analyses/:id/review`(**upsert**, `X-Review: CREATED|UPDATED`) · `GET /v1/ops/analyses/stats`(버전별 승인율·구조화 실패율). **few-shot**: 승인+ok 행을 별점순 3개, **대상 인시던트 자신 제외**, scrubText 후 system static 뒤에 격리 문구와 함께 → 예시가 실제로 들어가야만 `promptVersion='v2'`(풀이 비면 v1 그대로 — 이름표 오염 방지), body `fewShot:false` 가 v1 대조군. 앱 S5 `(tabs)/review.tsx` + `features/review/`(Pan 제스처 + Reanimated, 낙관적 업데이트는 실패한 카드 한 장만 복귀, `GestureHandlerRootView` 루트 감쌈) + S4 CTA. 평가 세트 스크립트 `backend/eval/ops-review-set.ts`(list/seed/test/stats — seed·test 는 **서로 다른 인시던트**, 블라인드 채점 후 stats 로 v1 vs v2). 결정 5건은 설계 §9 Phase 4. 검증: ops 단위 94 · e2e 22 · 양쪽 tsc · 실기기 18장 채점.
- **RN Ops Companion Phase 5 운영 배포 완료(2026-09-22, main `00107b7` = PR #37, 마이그레이션 `OpsToolCalls` 적용) — DoD ① 은 "읽는다"까지만**: 소스 코드를 읽는 분석(tool use). `read_source` 도구(`ops/source-reader.service.ts` — GitHub raw 무인증 + Redis 캐시 `ops:src:<ref>:<path>`, 허용 폴더 `backend/src`·`frontend/src`·`ops-companion/(app|src)` 만, `.env*`·`*.pem` 이름 거절, 80줄/회·3회/분석, 실패는 `{ok:false,reason}`) → `OpsAnalysisService` 가 `generateWithTools` 로 호출(마지막 라운드 텍스트만 답, 교정 재시도는 도구 없이). **버전 v3 = 도구가 프롬프트에 들어간 분석**(few-shot 끔, 호출 여부는 `ops_analyses.tool_calls` — `null`=미제공·`[]`=미호출). 마이그레이션 `OpsToolCalls1790026688606`(로컬·운영 적용). 상한은 **분당 LLM 호출 수** 예약형 `RedisService.reserveRateLimit`(`OPS_ANALYSIS_MAX_LLM_PER_MIN` 기본 12, 도구 켬 5·끔 2 예약; 옛 `OPS_ANALYSIS_MAX_PER_MIN` 은 무시+경고). **발견 3건**: ① `webpack.config.js` `sourceMaps` 오타로 **운영 빌드에 소스맵이 없었다** → `sourceMap` 수정 + Dockerfile `CMD node --enable-source-maps`(로컬 실험: 스택이 `webpack://shopping-mall/backend/src/main.ts:60` 으로) ② 프론트는 Vercel 소스맵 업로드가 없어 프레임이 청크 좌표(범위 밖) ③ 백엔드 Sentry 이벤트에 release 가 없었다 → `instrument.ts` `release: APP_VERSION`. 상세 응답에 `release`·`firstRelease`. 실측: 앱 인시던트 v3 가 실제로 2파일 읽고 정답(#29); **Phase 4 test 세트 6건은 전부 번들 좌표라 도구 호출 0회**(확신도만 하락); **배포 후 CORS(봇과 같은 Origin=서버 자신)는 `main.ts:45-75@00107b7` 를 읽고도 "CORS_ORIGINS 에 추가하라" 오답 그대로(#43)** — 그 도메인이 서버 자신이라는 사실은 소스에 없다(배포 맥락). **네 번째 시도 (a) 서비스 지도(PR #38 → main `7e3784f`, 2026-09-22 운영 재배포, 마이그레이션 없음)**: `SERVICE_MAP`(배포 구성 사실 4줄, 결론 없음) 을 system 에 → 버전 `.1`(`OPS_ANALYSIS_SERVICE_MAP` 기본 켬, body `serviceMap`) → 같은 CORS 이벤트를 v3.1(#45)·v1.1(#46) 모두 정답("봇이 서버 자신 도메인을 Origin 으로, 정상 차단"). 도구의 몫 = 지어내지 않음(v1.1 은 조치 코드를 지어냄). 실기기 채점: v3 8/10(test 6건 6/6, CORS #43·#44 반려) · **v3.1 7/7(test 6건 6/6 + CORS #45 승인)** — 지도는 CORS 를 고치고 다른 답을 망치지 않았다. 별점은 코드를 지어낸 v1.1(#46, 4점)이 지어내지 않은 v3.1(#45, 2점)보다 높았다(별점 = 구체성?). **Phase 5 종료.** 운영 분석은 이제 기본 v3.1. 다음 = Phase 6(아래).
- **RN Ops Companion Phase 6 — 운영 배포 완료(2026-09-22, main `77c4f19` = PR #39, 마이그레이션 없음)**: 프론트 소스맵 업로드 + 새 평가 세트. **Vercel 에 `SENTRY_ORG`·`SENTRY_PROJECT`·`SENTRY_AUTH_TOKEN` 3개 + Redeploy(사용자)** → 프론트 프레임이 `./src/hooks/useCategories.ts:9` 꼴(저장소 폴더 이름 없음 · 라이브러리는 `../node_modules/…`) → `SourceReaderService.normalizeFramePath(filename, project?)` 에 **Sentry 프로젝트 slug 힌트**(`PROJECT_ROOTS`, 폴더 이름이 없을 때만 `frontend/` 를 붙이고 `checkPath` 로 검증). 소스맵은 릴리즈 파일이 아니라 **Debug ID 아티팩트 번들**로 올라간다 — `releases/<sha>/files/` 0개는 미업로드의 증거가 아니다. `Network Error`·5xx 는 axios 가 던져 소스맵이 있어도 우리 프레임 0 → 재료는 **프로브**(헤드리스 Chrome + `page.route` 로 API 응답 항목만 깨기, 서버 무접촉)로 만든 프론트 이슈 5건 + 카테고리 1건. 스크립트 `list --readable`(읽을 수 있는 파일 수 열) · `stats --after <id>`(`getStats({minAnalysisId})` — 옛 v1.1·v3.1 행 분리). **세트 7건 × v1.1(지도만) vs v3.1(지도+도구) = 14/14 ok(#54~#67), v3.1 7건 모두 도구 호출** — 도구 유무만 다른 첫 비교. seed 없음(few-shot 팔 없음). DoD ①② 통과(#53 이 `frontend/src/hooks/useCategories.ts:1-29@7e3784f` 를 읽고 정답), **DoD ③ 채점 결과(14장 블라인드): 승인율 v1.1 7/7 = v3.1 7/7(천장 — 축이 못 됨), 별점 3.43 → 4.29, 쌍별 v3.1 5승 1무 1패. 6편과 반대로 별점이 도구 쪽 — v1.1 은 또 `FRONTEND_URL` 코드를 지어냈고(4점) v3.1 은 실제 줄 인용(5점). 표본 7쌍·평가자 1명. ⚠ 타당성 한계: 평가자가 "상황·해답을 몰라 임의 승인한 건이 많다"고 진술 → 승인 14/14 는 "틀렸다고 볼 근거가 없었다"(설계 문제)**. 다음 = **Phase 7(사용자 승인)**: 인시던트별 사실 메모 + 확인 항목 4개(원인 위치·지어낸 식별자 없음·그대로 적용 가능·확신도 적정)를 카드에 → 승인/반려 파생 → 14장 재채점(upsert)으로 안내 전/후 비교. 메모 초안 7건은 설계 §9 Phase 7 표. DB 변경 0 · 앱 변경 0 · 새 비밀값 1(Vercel 에만). 검증: ops 단위 159 · tsc. 부수 발견: CSP `worker-src` 부재로 Sentry Replay 워커 차단 · `flattenTree`·`ProductCard` 배열/null 가드 없음. 운영 확인: 실기기에서 `frontend/src/hooks/useCategories.ts:1-29` 를 읽음(조치 정답, 원인 서술 반쯤 — Phase 7 항목 ① 의 예). **Phase 6 종료.** 인수인계 `docs/roadmap/_next-session-phase7.md`(Phase 7), 상세 `docs/learning/ops-companion/07-frontend-sourcemaps-and-eval-set.md`, 설계 §9 Phase 6. 운영 이벤트는 이제 `webpack://shopping-mall/backend/src/main.ts:60` + `release: 00107b7`. 검증: ops 단위 142 · e2e 22 · 양쪽 tsc. 상세 `docs/learning/ops-companion/06-source-reading.md`.

**비어 있음 / 스켈레톤**
- **셀러 프론트** `(main)/seller/*` 중 stub: 대시보드/문의. (상품 목록·등록·수정, 주문/배송, 정산은 실구현, 신청 화면 `my/seller-apply`도 실구현)
- **관리자 하위 페이지** stub: `categories`만. (dashboard·audit-logs·sellers·products·orders·settlements는 실구현)
