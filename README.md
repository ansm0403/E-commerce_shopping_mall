...
저는 원래 프론트엔드 개발자라서 이번 프로젝트가 백엔드를 제대로 다뤄보는 첫 경험이었습니다. 아래는 NestJS를 포함해 **이번에 처음 써본 백엔드 스택들을 왜 골랐는지**에 대한 솔직한 이유입니다.

### NestJS

백엔드를 처음 시작하는 입장에서, 그래도 익숙한 TypeScript를 쓸 수 있는 Node.js 기반이면 좋겠다고 생각했습니다. 동시에 자바 스프링에서 자주 언급되는 의존성 주입(DI), 데코레이터 같은 개념도 경험해보고 싶었는데, NestJS가 딱 그 사이에 있는 느낌이었습니다. Node.js의 친숙함은 그대로 가져가면서 스프링과 비슷한 구조와 개념을 배울 수 있어서 선택했습니다.

### PostgreSQL

이커머스는 유저-주문-상품-결제처럼 데이터끼리 관계가 복잡하게 얽혀 있어서 관계형 DB가 맞다고 생각했습니다. 그중 PostgreSQL을 고른 이유는, 카테고리마다 다른 스펙 필드(예: 의류는 사이즈, 식품은 유통기한)를 JSONB 컬럼으로 유연하게 저장할 수 있다는 점이 좋아 보였기 때문입니다. 정해진 형식의 데이터와 자유로운 형식의 데이터를 한 DB에서 같이 다룰 수 있다는 게 매력적이었습니다.

### Redis

이메일 인증 코드처럼 "일정 시간이 지나면 자동으로 사라져야 하는 데이터"를 다룰 일이 생겼는데, 이걸 일반 DB에 넣고 만료 처리를 직접 하는 건 번거로워 보였습니다. Redis는 TTL(만료 시간)을 기본으로 지원해서 이런 용도에 잘 맞았고, 나중에 관리자 대시보드 조회 결과를 캐싱하는 데에도 써보면서 인메모리 저장소가 어떤 건지 직접 경험해볼 수 있었습니다.

### JWT

프론트엔드만 할 때는 토큰을 받아서 헤더에 넣는 정도만 해봤는데, 정작 그 토큰이 어떻게 발급되고 검증되는지는 잘 몰랐습니다. 프론트엔드와 백엔드가 분리된 구조라 서버가 세션을 직접 들고 있지 않아도 되는 방식이 필요했고, JWT를 발급하고 검증하는 쪽을 직접 구현하면서 그동안 "받기만 하던" 토큰의 반대편을 이해할 수 있었습니다.

### Docker

"제 컴퓨터에서는 잘 되는데요" 문제를 겪고 싶지 않았습니다. 로컬에서 개발할 때와 EC2에 올렸을 때 PostgreSQL, Redis 같은 환경이 똑같이 맞춰지길 원했고, Docker로 환경을 통째로 묶어두면 그게 가능하다는 걸 알게 되어 도입했습니다. 처음에는 낯설었지만 한 번 익혀두니 배포가 훨씬 마음 편해졌습니다.

### AWS

프론트엔드 배포는 Vercel처럼 알아서 다 해주는 환경에 익숙했는데, 이번에는 서버를 직접 다뤄보고 싶었습니다. AWS EC2로 실제 리눅스 서버에 접속해서 직접 컨테이너를 띄우고, 메모리가 부족하면 인스턴스 사양을 올려보는 식으로 트러블슈팅을 해보면서 "배포가 알아서 되는 것"이 아니라는 걸 몸으로 배울 수 있었습니다.
