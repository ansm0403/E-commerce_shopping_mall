 ...
 *   (이름 대조 칩은 features/analysis/IdentifierChip.tsx — S4·S5 공용, AnalysisCard 의 추천 조치 아래에 그린다)
...
import type { IncidentNote, ReviewCheckKey, ReviewChecklistItem, ReviewChecks, ReviewVerdict } from '../../lib/api';
...
export function GuidancePanel({ note, mode = 'review' }: { note: IncidentNote | null; mode?: 'review' | 'detail' }) {
  const [open, setOpen] = useState(true);
  const [codeOpen, setCodeOpen] = useState(false);
  // S4(detail): 메모는 "사람이 조사해 확정한 원인·조치"다. 없는 인시던트에 "메모 없음"을 보일 이유가 없다 — 칸 자체를 뺀다(훅 뒤에서 return)
  if (mode === 'detail' && !note) return null;

  return (
    <View style={styles.panel}>
      <Pressable style={styles.header} onPress={() => setOpen((v) => !v)} accessibilityRole="button" accessibilityLabel="채점 안내 접기/펴기">
        <Text style={styles.headerTitle}>{mode === 'detail' ? '사람이 확인한 운영 메모' : '채점 안내 · 사실 메모'}</Text>
        ...
        note ? (
          <View style={styles.body}>
            <Text style={styles.lead}>
              {mode === 'detail'
                ? '이 인시던트는 사람이 조사해 원인과 조치를 확정했습니다. 아래 AI 답과 다르면 이 메모를 따르세요.'
                : '사람이 미리 확인한 정답입니다. 아래 AI 답을 이 메모와 대조해 채점하세요.'}
            </Text>
              ...
              <>
                ...
                {codeOpen ? (
                  ...
                  <Text style={styles.codeHint}>
                    {mode === 'detail'
                      ? '원인 위치의 실제 코드입니다. AI 조치를 붙여 넣기 전에 이름과 줄을 여기서 확인하세요.'
                      : '항목 ②(지어낸 이름)를 볼 때 펴세요. AI 조치 코드의 변수·함수 이름이 여기 있는지 대조합니다.'}
                  </Text>
                )}
              </>
            ) : null}
