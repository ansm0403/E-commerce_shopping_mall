...
> 기준 시점: 2026-09-22 (커밋 `454fae0`, Phase 4)
...
flowchart TB
    ...
    Shop -- "에러 이벤트" --> Sentry
    Shop -- "빌드 시 소스맵 업로드(Vercel 환경변수의 업로드 토큰, Phase 6)" --> Sentry
    Phone -- "앱 자신의 에러(DSN)" --> Sentry
    API -- "2분마다 조회(API 토큰)" --> Sentry
    API -- "인시던트 분석 요청(LLM 키)" --> LLM
    API -- "스택의 파일:줄 읽기(무인증 raw, 커밋 기준)" --> GH
...
sequenceDiagram
    ...
    alt 있음
        ...
        API->>LLM: "이 스키마의 JSON 으로만 답하라" + 인시던트 + read_source 도구 (LLM 키)
        loop 모델이 read_source 를 요청하면 (분석당 최대 3회)
            API->>GH: 파일의 줄 범위 (커밋 = 이벤트 릴리즈, 없으면 main · Redis 캐시)
            GH-->>API: 코드 조각 (또는 404·상한 → 실패 사유)
            API->>LLM: 도구 결과 (마스킹 · 줄 번호)
        end
        LLM-->>API: 텍스트
        API->>API: JSON 검증 — 어겼으면 사유를 실어 1회 더
        API->>PG: 저장 (ok | parse_failed, 프롬프트 버전, 걸린 시간, 읽은 파일 기록)
        API-->>App: 결과
    end
```
...
> **tool use(도구 호출)**: LLM 에게 "이런 함수를 부를 수 있다"고 알려 두면, 모델이 답 대신 "이 함수를 이 인자로 실행해 달라"는 요청을 돌려주는 방식. 실행은 항상 **우리 백엔드 코드**가 하고, 결과 문자열만 모델에게 되돌아간다. 모델이 인터넷에 직접 닿는 일은 없다. 쇼핑몰 관리자 어시스턴트의 매출 조회 도구와 같은 구조다.

**모델에게 눈이 하나 생겼다 — 소스 코드.** 4편의 첫 분석이 CORS 를 틀린 이유는 모델이 `main.ts` 의 `enableCors` 를 볼 수 없어서였다. 그래서 분석에 `read_source(path, startLine, endLine)` 도구를 준다. 모델이 스택트레이스의 파일:줄 주변을 읽어 달라고 하면 백엔드가 **GitHub 의 raw 파일**을 받아 줄 범위만 잘라 돌려준다(위 그림의 loop). 이 연결이 이 문서에 새로 생긴 외부 선이고, 세 가지 규칙이 3-1 의 원칙을 잇는다.

- **① 비밀값 0** — 저장소가 public 이라 무인증으로 읽는다(시간당 60회/IP). 토큰이 없으니 유출될 것도 없다. 대신 백엔드가 **읽을 수 있는 폴더를 제한**한다(`backend/src`·`frontend/src`·`ops-companion/app|src` 만, `.env*`·`*.pem` 같은 이름은 거절) — public 이어도 "모델이 원하는 파일을 아무거나 읽는 구조"는 만들지 않는다. 한 번에 80줄, 분석당 3회.
- **② 어느 시점의 코드인가** — 백엔드는 Sentry 이벤트에 **릴리즈 = 배포 커밋(짧은 SHA)** 을 적는다(`instrument.ts` 의 `release: APP_VERSION`). 릴리즈가 커밋 꼴이면 그 커밋의 파일을, 아니면(앱의 `1.0.0+N`, 옛 이벤트) `main` 을 읽되 프롬프트에 "발생 시점과 다를 수 있다"고 적어 확신도를 낮추게 한다. 같은 커밋의 파일은 불변이라 Redis 에 7일 캐시한다.
- **③ 번들 좌표를 원본으로** — 운영 백엔드의 스택은 원래 `/app/backend/dist/main.js:17026` 처럼 **하나로 합쳐진 파일**의 줄이었다(2-6 의 소스맵이 앱에 필요한 것과 같은 이유). Phase 5 부터 이미지가 소스맵을 갖고 `node --enable-source-maps` 로 뜨므로 스택이 `backend/src/main.ts:65` 로 찍힌다. Sentry 에 올리는 방식(앱)과 달리 **서버 자신이 변환**한다 — 업로드 토큰·CI 절차가 늘지 않는다. 쇼핑몰 프론트는 브라우저에서 실행되므로 서버 방식이 불가능하고, Phase 6 에서 **앱과 같은 업로드 방식**을 택했다 — Vercel 빌드가 `withSentryConfig` 로 소스맵을 올리고(Vercel 환경변수 `SENTRY_AUTH_TOKEN`), 프레임은 `./src/hooks/useCategories.ts` 꼴이 된다. 이 꼴에는 저장소 폴더 이름(`frontend/`)이 없어 백엔드가 Sentry 프로젝트 slug 를 힌트로 붙인다([7편 3-1](./07-frontend-sourcemaps-and-eval-set.md#3-1-normalizeframepath--폴더-이름이-있으면-그대로-없으면-힌트)).

읽은 파일은 `ops_analyses.tool_calls` 에 남는다(파일·줄·성공 여부만, 코드 원문은 저장하지 않는다 — GitHub 에 있다). 앱의 분석 카드는 이 기록을 "AI 가 읽은 코드" 칩으로 그린다. **도구가 실패해도 분석은 끝난다** — 파일이 없거나 GitHub 가 죽으면 모델에게 실패 사유가 돌아가고, 모델은 코드 없이 답하되 확신도를 낮춘다. 도구가 프롬프트에 들어간 분석의 버전이 `v3` 다(few-shot 은 끈다 — v1 과 "도구 하나만 다른" 비교).

**Postgres 표가 둘 늘었다.** 4-5 의 표 셋(푸시)에 `ops_analyses`(분석 결과)와 `ops_reviews`(사람의 판정)가 더해져 다섯이다. `ops_analyses` 는 Sentry 이슈 id 로만 이어지고 사용자 FK 가 없다 — 분석은 누가 요청했든 같은 이슈의 같은 답이기 때문이다. `ops_reviews` 는 반대로 **평가자 FK 가 있다** — 판정은 사람의 것이고, 한 사람이 한 분석에 남기는 판정은 하나다(유니크). Phase 4 까지 인프라는 늘지 않았고, Phase 5 에서 **외부 연결이 하나(GitHub) 늘었다** — 그래도 새 비밀값도, 앱의 새 네이티브 패키지도 없다.
...
| Sentry **업로드 토큰**(프론트용, Phase 6) | **Vercel 환경변수**(`SENTRY_AUTH_TOKEN`, `NEXT_PUBLIC_` 아님) | ❌ (빌드 서버에만 — 브라우저 번들에 안 들어간다) | 같음. expo.dev 의 것과 **값이 다른** 별도 토큰 — 한쪽이 새도 다른 쪽은 무사하다 |
...
| GitHub 소스 읽기 | **비밀값 없음** — public 저장소를 무인증 raw 로 읽는다(3-4) | — | 잃을 것이 없다. 저장소를 private 으로 바꾸면 read-only 토큰이 백엔드 `.env` 에 하나 는다 |
...
| **GitHub**(raw 읽기) 또는 시간당 60회 상한 | 아무것도 멈추지 않는다 — 도구가 "읽기 실패" 사유를 모델에게 돌려주고 **분석은 코드 없이 끝난다**(4편의 v1 처럼, 확신도만 낮아진다). 기록에 `ok:false` 로 남는다 | 분석 자체·목록·상세·푸시 전부 |
| **expo.dev 업로드 토큰 만료** | 새 빌드의 소스맵 업로드(스택이 압축 좌표로 남는다) | 나머지 전부. 빌드 자체는 성공한다 |
| **Vercel 업로드 토큰 만료**(Phase 6) | 새 프론트 배포의 소스맵 업로드 — 프론트 인시던트가 다시 "읽을 수 있는 파일 없음"이 된다 | 나머지 전부. `withSentryConfig` 는 토큰이 없으면 조용히 스킵하므로 **빌드는 성공**한다 — 눈치채기 어렵다 |
...
| 소스 코드 읽기 | **백엔드가 GitHub raw 를 무인증으로**, 커밋 기준, 폴더 허용 목록 | GitHub 토큰 · 검색 도구 추가 | public 이라 토큰 없이 된다(시간당 60회 + 캐시로 충분). 스택에 파일:줄이 이미 있어 검색 도구는 왕복만 늘린다 |
| 백엔드 번들 좌표 복원 | **이미지 안에서 `node --enable-source-maps`** | Sentry 에 백엔드 소스맵 업로드 | Dockerfile 한 단어 + webpack 옵션 오타 수정. 업로드 방식은 토큰·CI 절차가 늘고, 옛 이벤트는 어느 쪽도 못 되살린다 |
...
- Gemini: 무료티어 **분당 15회**. 관리자 어시스턴트와 앱의 AI 분석이 나눠 쓴다(분석은 **분당 LLM 호출 12회**로 자체 제한 — 도구를 켠 분석 한 건이 최악 5회를 미리 예약한다)
- GitHub raw: 무인증 **시간당 60회/IP**. 분석당 최대 3회 + 커밋별 7일 캐시라 닿지 않는다
...
| **아티팩트 번들** | Debug ID 방식에서 소스맵이 올라가는 묶음. 릴리즈에 파일이 매이지 않으므로 "릴리즈 파일 0개"는 업로드 실패의 증거가 아니다 ([7편 1-1](./07-frontend-sourcemaps-and-eval-set.md#1-1-아티팩트-번들artifact-bundle--릴리즈-없이-소스맵을-짝짓는-방식)) |
...
| **tool use(도구 호출)** | LLM 에게 부를 수 있는 함수를 알려 주고, 모델의 "실행해 달라" 요청을 우리 코드가 실행해 결과를 되돌리는 방식. 우리 도구는 `read_source` 하나 (3-4) |
| **`--enable-source-maps`** | Node 실행 옵션. 에러 스택을 소스맵으로 되돌려 원본 파일·줄로 찍는다. 백엔드 이미지가 이걸로 뜬다 (3-4) |
...
| **릴리즈** | Sentry 가 '어느 빌드에서 일어난 일인가'를 가리키는 이름표. 쇼핑몰 프론트는 커밋 SHA(40자, Vercel 자동), 백엔드는 배포 커밋의 짧은 SHA(Phase 5 부터), 앱은 `패키지@버전+versionCode`. AI 분석은 이 값으로 "어느 커밋의 코드를 읽을지" 정한다 (2-7 · 3-4) |
...
| **프롬프트 버전** | 각 분석이 어느 문구로 만들어졌는지 남기는 표식. `v1`=예시 없음, `v2`=승인된 예시 포함, `v3`=소스 읽기 도구 포함(예시 없음) (3-4) |
