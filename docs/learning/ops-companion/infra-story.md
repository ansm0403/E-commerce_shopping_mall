...
> 기준 시점: 2026-09-21 (커밋 `5e8ea91`, Phase 2 종료)
...
⚠ 조회할 때 **어느 Sentry 프로젝트인지 반드시 지정한다.** 빼면 조직 전체가 합산돼 쇼핑몰 웹의 세션이 앱의 건강 지표에 섞인다(실측: 지정하면 그룹 1개, 빼면 9개).
...
| 2026-09-21 | `5e8ea91` | Phase 2 종료. **expo.dev → Sentry 소스맵 업로드 선 추가**(0-1 지도 · 2장 흐름 · 2-6 신설) · **versionCode/릴리즈**(2-7 신설) · 빌드 프로필 표에 소스맵·versionCode 열 · **Release Health** 를 역할 A·B 순환으로(4-6) · **생체 잠금은 인프라를 늘리지 않는다**(3-3) · 비밀값 지도에 업로드 토큰·지문 정보 2줄 + "앱에 비밀을 두지 않는다"의 정확한 뜻 · 의존성 표에 업로드 토큰 만료 · 용어 8개 추가 |
