...
- **③ 번들 좌표를 원본으로** — 운영 백엔드의 스택은 원래 `/app/backend/dist/main.js:17026` 처럼 **하나로 합쳐진 파일**의 줄이었다(2-6 의 소스맵이 앱에 필요한 것과 같은 이유). Phase 5 부터 이미지가 소스맵을 갖고 `node --enable-source-maps` 로 뜨므로 스택이 `backend/src/main.ts:65` 로 찍힌다. Sentry 에 올리는 방식(앱)과 달리 **서버 자신이 변환**한다 — 업로드 토큰·CI 절차가 늘지 않는다. 쇼핑몰 프론트는 Vercel 에 소스맵 업로드가 없어 아직 압축 좌표다(범위 밖).
...
| 2026-09-22 | `feat/ops-source-reading` | Phase 5. **GitHub 노드와 선**(0-1 지도) · **3-4 에 tool use 문단 + 시퀀스의 도구 loop**(read_source · 폴더 허용 목록 · 커밋 기준 읽기 · `--enable-source-maps`) · 5장에 "GitHub — 비밀값 없음" 행 · 6장에 GitHub 행(죽어도 분석은 코드 없이 끝난다) · 7장에 소스 읽기·번들 좌표 복원 결정 2행 + GitHub 상한 · 용어 3개(tool use · `--enable-source-maps` · 릴리즈 정의 갱신) · 8장에서 소스 읽기 행 제거. 새 서비스 1(GitHub, 무인증) · 새 키 0 |
