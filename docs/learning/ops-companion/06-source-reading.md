...
> 작성 시점: 2026-09-22 (`d15027d` → main `00107b7` = PR #37 → 서비스 지도 `f4df4a3` → main `7e3784f` = PR #38 — **운영 배포·실기기 채점까지 완료**)
...
**AI 에게 `read_source` 도구 하나를 줘서 스택트레이스가 가리키는 파일:줄을 GitHub 에서 직접 읽고 답하게 했다. 앱 인시던트에서는 코드 두 조각을 읽고 맞혔고, 배포 뒤 CORS 이슈에서는 `main.ts` 를 실제로 읽고도 같은 오답을 냈다 — 코드에 없는 사실(그 도메인이 서버 자신)은 코드를 읽어도 모른다. 그 과정에서 운영 빌드에 소스맵이 아예 없었다는 것과 프론트 소스맵이 업로드된 적이 없다는 것을 발견했다.**
...
| 백엔드 스택을 원본 좌표로 — webpack `sourceMap` 오타 수정 + `node --enable-source-maps` | ✅ 로컬 실험 · ✅ **운영**(2026-09-22 배포 `00107b7` 직후 만든 CORS 이벤트: 프레임 `webpack://shopping-mall/backend/src/main.ts:60`, 릴리즈 `00107b7`) |
| Sentry `release` = 배포 커밋(짧은 SHA) · 이벤트 `release`·`firstRelease` 를 상세와 프롬프트에 | ✅ 단위 · e2e(키 목록) · ✅ 운영 이벤트 `release: 00107b7` |
...
| 앱 카드 "AI 가 읽은 코드" 섹션 + 메타 줄 "(코드 n)" | ✅ tsc · ✅ 실기기(CORS 카드: 칩 `main.ts:45-75`, "v3 (코드 1)") |
| DoD ① CORS 이슈를 v3 가 실제로 읽고 오답을 내지 않는다 | **절반** — 읽기는 ✅(`tool_calls`: `backend/src/main.ts:45-75 @00107b7`), 오답은 ❌ **그대로**("`CORS_ORIGINS` 에 `api.ansmoon.dev` 를 추가하라", high/high). 아래 "실측 두 번째" |
| DoD ② v1·v2·v3 승인율 표 | ✅ **실기기 채점 9건(2026-09-22)** — 아래 표. v3 test 6건 6/6, 전체 8/9(88.9%). 유일한 반려 = #43(CORS 오답) |
| DoD ③ 도구 실패에도 분석이 v1 처럼 끝난다 | ✅ 단위(파일 없음·범위 밖·상한·모르는 도구·LLM 장애) · 실측(6건 모두 "읽을 파일 없음" 상태에서 ok) |
| **네 번째 시도 (a) — 서비스 지도** `SERVICE_MAP`(배포 구성의 사실 4줄, 결론 없음)을 system 에 · 버전 `.1` · `OPS_ANALYSIS_SERVICE_MAP` · body `serviceMap` · 스크립트 `--arms v1.1,v2.1,v3.1` | ✅ 단위 4건(합 146) · **실측: 같은 CORS 이벤트를 v3.1(#45, `main.ts:40-80` 읽음)·v1.1(#46, 도구 없음) 모두 "봇이 서버 자신의 도메인을 Origin 으로 — 정상 차단, Sentry 필터" 정답(low/high)**. 아래 "실측 세 번째" |
...
**실측 수치(2026-09-22, 로컬 DB, 평가자 1명, flash-lite)**

| 집합 | v1 | v2 | v3 | v3.1(지도) |
|---|---|---|---|---|
| test 6건(같은 인시던트, 블라인드) | 5/6 = 83.3% | 4/6 = 66.7% | **6/6** | **6/6** |
| CORS, Origin = 서버 자신 | 0/1(#14 반려) | — | 0/2(#43·#44 반려) | **1/1**(#45 승인, 별점 2) |
| 전체(`stats`, 옛 행·seed 포함) | 8/12 = 66.7% | 4/6 = 66.7% | 8/10 = 80.0% | **7/7 = 100%** |
| 평균 별점 | 3.88 | 3.75 | 3.13 | 3.29 |
| 도구를 실제로 호출한 분석 | 0 | 0 | 4(#29·42·43·44) | 1(#45) |

v1.1(지도만, #46) 은 1건 승인·별점 4. 코드를 지어낸 #46 이 4점, 지어내지 않은 #45 가 2점을 받았다 — 평가자에게는 "구체적인 코드 조치"가 있는 쪽이 더 쓸모 있어 보인 것이다. 별점이 "정확함"보다 "구체성"을 재고 있을 수 있다는 뜻이고, 5편 6-6 의 "별점을 믿지 말라"와 이어진다.

v3 가 test 6건에서 전부 승인됐지만, 그 6건은 도구를 한 번도 부르지 않았다(6-4) — 승인된 것은 "코드를 읽은 답"이 아니라 "읽을 수 없다고 확신도를 낮춘 답"이다(6-5). 별점이 내려간 것(3.88 → 3.13)이 그 대가로 보인다. v3 의 유일한 반려는 #43, 즉 도구가 코드를 읽고도 낸 CORS 오답이다. 표본 6~9건이라 단정할 수 없는 것은 5편과 같다.

**실측 두 번째 — CORS, 읽고도 틀렸다(2026-09-22, 배포 직후, 로컬 백엔드 → 같은 Sentry 이슈 → GitHub `00107b7`).** 배포 후 봇과 같은 요청(허용되지 않은 Origin)을 두 번 보내 원본 좌표의 새 이벤트를 만들고 v3 로 분석했다.

| 이벤트의 Origin | 읽은 것 | 답 |
|---|---|---|
| `https://phase5-check.invalid`(임의) | `backend/src/main.ts:45-75 @00107b7` | medium/high · "허용 목록에 없는 출처를 차단하는 정상 동작 — 스캔 도구나 오설정 클라이언트" · 조치는 `cb(new Error)` 대신 `cb(null,false)` 로 500 을 피하라. **맞다**(#42) |
| `https://api.ansmoon.dev`(봇이 실제로 보내는 값 = 서버 자신) | 같은 범위 | high/high · "`CORS_ORIGINS` 에 `https://api.ansmoon.dev` 를 추가하거나 정당한 요청인지 확인하라". **4편 6-8 · 5편 #14 와 같은 오답**(#43) |

**실측 세 번째 — 지도 한 단락으로 풀렸다(2026-09-22, 같은 이벤트, 같은 커밋).** 코드에 없는 사실을 프롬프트에 직접 줬다. `SERVICE_MAP` 은 네 줄이다 — 백엔드 공개 주소는 `api.ansmoon.dev` 하나이고 서버 자신이라 정당한 요청의 Origin 에 올 수 없다 · 프론트는 Vercel 도메인이고 `CORS_ORIGINS` 는 그 하나뿐이며 값은 환경변수에 있다 · 앱은 Origin 을 보내지 않는다 · 봇·스캐너 요청이 매일 들어온다. **"그러므로 CORS 오류는 정상 차단이다"는 적지 않았다** — 그 결론은 모델이 내려야 측정이 된다.

| 팔 | 읽은 것 | 답 |
|---|---|---|
| v3.1(도구 + 지도, #45) | `backend/src/main.ts:40-80 @00107b7` | low/high · "외부 스캐너가 API 서버 도메인을 Origin 으로 설정 … 보안 설정이 의도대로 동작" · 조치 = Sentry Inbound Filter 또는 전송 제외. **정답**, 코드 수정 제안 없음 |
| v1.1(지도만, #46) | — | low/high · 같은 판단 · 조치 예시로 `allowedOrigins = [process.env.FRONTEND_URL]` 코드를 **지어냈다**(실제 코드는 `CORS_ORIGINS` 를 쉼표로 나눈다) |

지도만으로도 판단은 맞는다. 도구가 더한 것은 **지어내지 않는 것**이다 — 코드를 읽은 v3.1 은 없는 코드를 쓰지 않았다. 4편 6-8 → 5편 #14 → 6편 #43 → #45, 한 인시던트의 네 번째 시도에서 처음 맞았다. 부작용도 쟀다 — test 세트 6건을 v3.1 로 다시 만들어(#47~#52, 전부 도구 0회) 채점하니 **6/6 승인**, v3 와 같다. 지도는 CORS 를 고치고 다른 답을 망치지 않았다(표본 6건).

두 번째가 원래의 함정이고, 도구는 함정을 풀지 못했다. 이유는 분명하다. `main.ts` 에는 허용 목록이 `CORS_ORIGINS` 환경변수에서 온다는 것까지만 있고, **`api.ansmoon.dev` 가 이 서버 자신의 주소라는 사실은 어디에도 없다**(nginx 설정·EC2 `.env`·Vercel 설정에 흩어져 있다). 코드를 읽어 "어느 줄이 차단했나"는 정확히 짚었지만(60~65행), "차단당한 이름이 누구인가"는 코드 밖의 배포 맥락이다. 이번 편의 결론은 그래서 두 줄이다 — **소스 읽기는 "어느 코드"의 문제를 풀었고, "그 이름이 누구"의 문제는 다른 재료(서비스 지도)가 필요하다.** 6-8 에 함정으로, 8장에 다음 후보로 적었다.
...
| 배포 이미지 | `CMD` 에 `--enable-source-maps` 한 단어 · `*.js.map` 8개 — 운영 `00107b7`(2026-09-22, 마이그레이션 `OpsToolCalls` 1건 적용, EC2 `.env` 무변경) |
| 앱 패키지 | **0** |
...
## 6-8. 코드를 읽고도 틀렸다 — 코드에 없는 사실은 코드로 못 배운다

배포 직후 봇과 같은 Origin(`https://api.ansmoon.dev`)으로 CORS 이벤트를 만들고 v3 를 돌렸다. 기록은 완벽했다 — `main.ts` 45~75줄을 그 커밋(`00107b7`)으로 읽었고, rootCause 는 "60~65행의 로직에 따라 허용 목록에 없는 오리진을 차단"이라고 정확히 짚었다. 그리고 조치는 여전히 "`CORS_ORIGINS` 에 `https://api.ansmoon.dev` 를 추가하라"였다(#43, high/high). 임의의 Origin 으로 만든 이벤트(#42)에서는 "정상 차단, 스캔 도구"라고 맞혔으니, 모델이 틀린 지점은 정확히 **그 이름이 서버 자신이라는 사실**이다. `main.ts` 는 허용 목록이 env 에서 온다고만 말한다. 도메인이 무엇을 가리키는지는 nginx 설정·EC2 `.env`·Vercel 에 흩어져 있고 저장소 소스 폴더에는 없다.

**교훈**: 도구는 "볼 수 있는 것"만 늘린다. 4편 6-8 의 오답을 "코드를 못 봐서"라고 진단한 것은 절반만 맞았다 — 나머지 절반은 **배포 맥락**이었다. 확신도가 high 로 돌아온 것도 눈여겨볼 점이다. 6-5 에서 근거가 없으면 낮추던 확신도가, 근거(코드)가 생기자 다시 올라갔다 — 근거가 **충분한지**는 모델이 판단하지 못한다.
...
- [x] 운영 배포 후 새 CORS 이벤트의 프레임이 `webpack://…/backend/src/main.ts` 이고 릴리즈가 커밋 SHA 다(`00107b7`)
- [x] 그 이벤트를 v3 로 분석 → `tool_calls` 에 `main.ts` — 읽기는 됐다
- [x] "서버 자신을 허용하라"는 오답이 없다 — v3 는 미통과(6-8), **서비스 지도를 넣은 v3.1(#45)·v1.1(#46)에서 통과**
- [x] 실기기: 카드에 "AI 가 읽은 코드" 칩(CORS `main.ts:45-75`, 메타 "v3 (코드 1)") · 평가 탭에서 v3 9건 채점 → `stats` 표 — **DoD ②**(앱의 "다시 분석"이 만든 #44 도 같은 오답)
...
이번 편은 도구를 만들고, 배포하고, 원래의 함정에서 한 번 더 틀리는 것까지 봤다. **DoD ① 은 "읽는다"까지만 통과했고, DoD ② 는 채점이 있어야 닫힌다.**

- **네 번째 시도 (a) 는 됐고 부작용도 없었다(v3.1 test 6/6).** `SERVICE_MAP` 은 기본 켬이라 앞으로의 모든 운영 분석이 `.1` 이다. 후보 (b)(배포 파일 읽기)는 하지 않는다. 지도는 손으로 쓴 사실이라 도메인·구성이 바뀌면 **사람이 고쳐야 한다** — 그때 접미사를 `.2` 로 올린다.
- **채점 → 표.** 로컬 DB 에 v3 가 9건(#29 앱, #30·31·35·39·40·41 test 세트, #42·#43 CORS)이다. 실기기에서 채점하면 5편의 v1·v2 와 나란히 v1·v2·v3 승인율 표가 나온다. 6-5 의 "확신도만 낮아진 답"이 승인되는지, #43 이 반려되는지가 관전 포인트다.
