...
> 작성 시점: 2026-09-22 (브랜치 `feat/ops-source-reading` `d15027d` → main `00107b7` = PR #37 — **운영 배포까지 완료**. 실기기 채점은 미확인, 0-3 표 참고)
...
| 집합 | v1 | v2 | v3 |
|---|---|---|---|
| test 6건(같은 인시던트, 블라인드) | 5/6 = 83.3% | 4/6 = 66.7% | **6/6 = 100%** |
| 전체(`stats`, 옛 행·seed·CORS 포함) | 8/12 = 66.7% | 4/6 = 66.7% | **8/9 = 88.9%** |
| 평균 별점 | 3.88 | 3.75 | 3.13 |
| 도구를 실제로 호출한 분석 | 0 | 0 | 4(#29·42·43·44) |
...
지도만으로도 판단은 맞는다. 도구가 더한 것은 **지어내지 않는 것**이다 — 코드를 읽은 v3.1 은 없는 코드를 쓰지 않았다. 4편 6-8 → 5편 #14 → 6편 #43 → #45, 한 인시던트의 네 번째 시도에서 처음 맞았다. 표본은 여전히 1건이고, 지도가 다른 인시던트를 망치지 않는지는 test 세트를 `.1` 로 다시 돌려 채점해야 안다(8장).
...
- **네 번째 시도 (a) 는 됐다 — 이제 부작용을 잰다.** `SERVICE_MAP` 은 기본 켬이라 앞으로의 모든 분석이 `.1` 이다. 지도가 CORS 는 고쳤지만 다른 인시던트를 망치지 않는지는 모른다. test 세트 6건을 `--arms v3.1` 로 다시 만들어 채점하면 v3 vs v3.1 이 나온다(6건 = 도구 5회 예약 × 6 → `--delay 31000`). 후보 (b)(배포 파일 읽기)는 (a) 로 충분하면 하지 않는다.
