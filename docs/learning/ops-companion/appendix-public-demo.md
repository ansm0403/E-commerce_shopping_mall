...
## 7. 웹 → 앱 연동 확인 — "내가 낸 에러가 앱에 뜬다" (2026-09-23)

preview APK 를 실기기로 돌려 본 관찰 하나가 이 절을 만들었다. "내용이나 볼륨은 좋은데 막상 실제 웹에서 에러를 발생할 방법을 사용자가 모르니까 웹과 앱이 연동이 되어있는지를 판단할 재료가 없었다. 오히려 내가 mock 데이터를 삽입하여 보여주기 식으로만 만든건가? 라는 생각조차 들 정도였다." 이 앱의 차별점은 "운영 데이터 → AI 분석 → 사람 채점" 의 순환 고리인데, 첫 고리 **"실제 장애가 들어온다"** 가 안 믿기면 나머지도 흔들린다. README 는 글로 주장하는 것이고, 이 기능은 방문자가 스스로 확인하는 것이다.

**무엇을 만들었나.** 웹 관리자 메뉴에 "운영 앱" 페이지(`/admin/ops-app`) — 앱 소개 → 설치(링크·QR·데모 계정) → **연동 확인** → 앱에서 할 일. 연동 확인의 버튼은 브라우저에서 진짜 에러를 만들어 프론트 Sentry 프로젝트로 보내고, 그 아래 추적기가 **앱과 같은 `GET /v1/ops/incidents`** 를 20초마다 최대 3분 물어 제목에 내 코드(`[방문자 테스트 A7K2]`)가 있는 항목이 나타나면 ✓ 와 첫 발생 시각을 보여준다. 폰이면 "앱에서 열기"(`opscompanion://incidents/<id>`, 푸시와 같은 경로).

**왜 백엔드 "테스트 에러 API" 가 아니라 브라우저에서 던지는가.** 실제 프론트 장애와 같은 길을 타야 "진짜" 다 — 프론트 프로젝트 → 소스맵 → 앱의 AI 분석(v3.1)이 `frontend/src/app/(admin)/admin/ops-app/visitor-test.ts` 를 GitHub 에서 읽고 "의도된 테스트 에러" 라고 답하는 장면까지 이어진다. 백엔드 API 로 던지면 프론트 소스맵 경로를 안 탄다. 이름 있는 함수 `raiseVisitorTestError` 에서 `new Error()` 를 만드는 것도 그래서다 — 스택 최상단 프레임이 그 파일이 된다. `throw` 하지 않고 `captureException` 만 부르는 이유는 `global-error` 화면으로 방문자를 내쫓지 않기 위해서.
...
**범위 밖으로 남긴 것.** 실제 상점 화면을 세션 단위로 고장 내는 카오스 플래그(운영 코드에 고장 스위치가 더 큰 흠), 대시보드 역방향 위젯(이력서 뒤), 서버 측 상한(이슈 하나당 이벤트 1건 — 브라우저 60초 쿨다운으로 충분).
