...
| 딥링크 — 종료(cold start) | ✅ 실기기. 앱을 완전히 종료한 상태에서도 탭 → 상세 직행 |
| 딥링크 — 미로그인(pending) | ✅ 실기기. 로그아웃 상태에서 탭 → 로그인 화면 → 로그인 후 그 상세로 |
| **Sentry 이슈 → 폰 푸시 → 탭 → 상세 진입** | ✅ 실기기(2026-09-20) — 아래 |
| 운영 배포 | ✅ main `6a609a9`. 마이그레이션 1건 적용, health version 단언 |
...
쿨다운도 실측했다. 커서를 되돌려 후보가 2건이 됐을 때 **발송은 1통**이었다 — 앞서 보낸 이슈가
6시간 쿨다운에 걸려 걸러진 것이다(3-3 의 선점 SQL 이 한 문장으로 판정한다).

종료 상태(cold start)도 통과했다. 다만 개발 빌드는 앱이 죽었다 켜질 때 PC 의 Metro 에서 번들을
다시 받으므로, **PC 가 꺼져 있으면 성립하지 않는다.** 데모 영상은 번들이 내장된 preview 빌드로
찍는 것이 정직하다(6-10).
...
## 6-11. 화면이 길어지면 버튼이 조용히 사라진다

프로필 화면에 푸시 카드를 더하자 아래쪽 **로그아웃 버튼이 화면 밖으로 밀려** 보이지 않았다.
스크롤이 없는 고정 레이아웃(`SafeAreaView` + `View`)이라 넘친 만큼이 그냥 잘렸다.
웹이라면 페이지가 늘어나며 스크롤바가 생기지만, RN 은 **명시적으로 `ScrollView` 로 감싸야** 스크롤된다.

→ [`app/(tabs)/profile.tsx`](../../../ops-companion/app/%28tabs%29/profile.tsx) 를 `ScrollView` 로 감쌌다.

**교훈**: 화면에 항목을 추가하면 작은 기기에서 무엇이 잘리는지 확인한다. 에러도 경고도 없이
기능이 사라지는 종류의 문제다.

## 6-12. `sound: 'default'` 는 기본 소리가 아니다
...
- [x] **앱을 완전히 종료한 뒤** 탭해도 상세로 들어간다 (cold start)
- [x] 로그아웃 상태에서 탭하면 로그인 화면이 뜨고, 로그인 후 그 상세로 이동한다
