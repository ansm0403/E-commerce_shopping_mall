...
- 결정(추천 1개, 위임 원칙): ① 에러는 **브라우저에서 진짜로**(`Sentry.captureException`, 백엔드 테스트 API 없음 — 실제 장애와 같은 길, AI 가 그 페이지 소스를 읽는다) ② 제목 `[방문자 테스트 A7K2] …` + `fingerprint: [prefix, code]` → **방문자마다 이슈 하나**, 코드가 앱에 그대로 나타나는 것이 "진짜" 증거 ③ "같은 에러를 다시 보내 평가 확인"은 불필요(Sentry 그루핑) — S4 의 `mine` 이 이미 데모에도 채워지는데 앱 카드가 `reviews===0` 이면 숨긴다 → **앱 한 줄** ④ 푸시 폴러는 접두어로 건너뜀(상수 `@shopping-mall/shared`), Slack 은 Sentry 알림 규칙 제목 필터(사용자) ⑤ 분석·채점은 데모 규칙 그대로, 이슈는 14일 뒤 목록에서 빠지고 Sentry auto-resolve(사용자) ⑥ 브라우저당 60초 쿨다운 ⑦ 두 번째 증거 = 앱 상세의 첫 발생 시각(버튼 시각과 일치).
- 범위: 웹 페이지 1개(`(admin)/admin/app-test`) + 메뉴 항목 · 백엔드 폴러 필터 + 단위 1건 · 앱 `ReviewSummaryCard` 한 줄(`eas update`) · README/부록. **DB 0 · 비밀값 0 · 네이티브 0.**
