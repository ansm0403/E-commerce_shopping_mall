...
| 후속 — S4 보강(2026-09-23 밤, 사용자 제안: "근거가 채점 화면에만 있고 고치는 화면엔 없다") | 분석 응답에 `project`·`note`·`identifierCheck`·`reviewSummary` 4필드 — **컨트롤러**(`enrichAnalysis`)가 채움(OpsAnalysisService 는 메모를 모른다, 격리 테스트 그대로) · `OpsReviewService.identifierCheckFor`(한 장 대조 — S4 는 블라인드 대상이 아니라 인시던트 합집합 불필요)·`summarizeReviews`(대기 카드엔 싣지 않음 — 블라인드) · 앱 S4 순서 **운영 메모(있을 때만) → 분석(+추천 조치 아래 칩) → 사람 채점 요약 → 평가하기** · `IdentifierChip` 을 `features/analysis/` 로 옮겨 S4·S5 공용 · `GuidancePanel mode="detail"`(조치용 문구, 메모 없으면 칸 없음) · `ReviewSummaryCard` 신규 | ✅ 단위 216 · e2e E·F 8 · 양쪽 tsc · 실 API(#67 메모·칩·승인 2/4.5, #55) · ✅ **운영 반영(2026-09-23)**: 이미지 `830a33b`(로컬 main 커밋) → push(사용자) → EC2 pull/up/reload → health `830a33b` · 부팅 로그 에러 0 · 새 라우트 401 · ⏳ 실기기 S4 확인(앱은 JS 만 — Metro `--clear` + 앱 강제 종료 후 재실행) |
...
### ✅ 웹 → 앱 연동 확인 — "내가 낸 에러가 앱에 뜬다" (2026-09-23 구현 완료, 브랜치 `feat/ops-web-app-link`. 운영 실측 ⏳ — 배포 후 아래 "진행" 에 기입)
- **진행(2026-09-23 구현)**: `shared/src/lib/ops/visitor-test.ts`(`OPS_VISITOR_TEST_PREFIX`·`buildVisitorTestTitle`·`isVisitorTestTitle`) · 백엔드 `ops/visitor-test.ts` + `OpsPollerService.isPushTarget` 제목 필터(+ 단위 1건, 폴러 스위트 23 통과) · 웹 `(admin)/admin/ops-app/`(`page.tsx` 소개·설치(링크·정적 QR `public/images/ops-app-install-qr.png`·데모 계정) + `components/LinkCheckSection.tsx` 버튼·추적기·딥링크·앱에서 할 일 + `visitor-test.ts` 에러 생성·전송·쿨다운) + 사이드바 "운영 앱" · 앱 `ReviewSummaryCard`(reviews 0 이어도 `mine` 있으면 표시). **구현에서 바뀐 것 2가지**: ① **백엔드는 shared 를 값으로 import 하지 않는다** — Nx webpack 이 buildable 워크스페이스 패키지를 `require('@shopping-mall/shared')` 로 외부화하는데 운영 이미지(`yarn workspaces focus --production`)의 node_modules 에 그 링크가 없다(이미지 안에서 `Cannot find module` 확인). 값은 백엔드에 한 번 더 적되 `import type` + `typeof` 로 shared 의 리터럴 타입에 묶어 tsc 가 불일치를 막는다(번들에 shared 참조 0 확인). ② **판정은 startsWith 가 아니라 includes** — Sentry 이슈 제목은 `VisitorTestError: [방문자 테스트 A7K2] …` 꼴이라 접두어가 맨 앞이 아니다. Sentry 알림 규칙도 "contains" 로. 추적기가 노출을 확인하면 상세(`/ops/incidents/:id`)에서 `firstSeen` 을 읽어 버튼 시각(KST)과 나란히 둔다(앱 상세는 "처음 N분 전" 상대 표기). 실측(버튼 → 앱 목록 노출까지 걸린 시간) 은 운영 배포 후 기입.
- 배경: preview APK 를 실기기로 돌려 본 사용자 관찰 — "웹에서 에러를 낼 방법을 방문자가 모르니 연동 여부를 판단할 재료가 없다. mock 을 심어 보여주기 식으로 만든 건가 싶을 정도". 순환 고리의 첫 고리("실제 장애가 들어온다")가 안 믿기면 나머지도 흔들린다. **README 갱신보다 먼저** 한다(README 에 이 한 줄이 들어가야 하므로).
- 결정(추천 1개, 위임 원칙): ① 에러는 **브라우저에서 진짜로**(`Sentry.captureException`, 백엔드 테스트 API 없음 — 실제 장애와 같은 길, AI 가 그 페이지 소스를 읽는다) ② 제목 `[방문자 테스트 A7K2] …` + `fingerprint: [prefix, code]` → **방문자마다 이슈 하나**, 코드가 앱에 그대로 나타나는 것이 "진짜" 증거 ③ "같은 에러를 다시 보내 평가 확인"은 불필요(Sentry 그루핑) — S4 의 `mine` 이 이미 데모에도 채워지는데 앱 카드가 `reviews===0` 이면 숨긴다 → **앱 한 줄** ④ 푸시 폴러는 접두어로 건너뜀(상수 `@shopping-mall/shared`), Slack 은 Sentry 알림 규칙 제목 필터(사용자) ⑤ 분석·채점은 데모 규칙 그대로, 이슈는 14일 뒤 목록에서 빠지고 Sentry auto-resolve(사용자) ⑥ 브라우저당 60초 쿨다운 ⑦ 두 번째 증거 = 앱 상세의 첫 발생 시각(버튼 시각과 일치). **2차 검토(같은 날)에서 추가**: ⑧ 페이지는 "테스트"가 아니라 **"운영 앱" 소개 페이지**(`/admin/ops-app` — 설치·데모 계정·연동 확인이 한 곳) ⑨ 버튼 아래 **진행 추적기**(보냄 → Sentry 수집 → 앱 목록 노출 ✓) — 웹이 앱과 같은 `GET /ops/incidents` 를 20초마다 최대 3분 폴링해 내 코드가 보이면 ✓ + 첫 발생 시각 ⑩ 노출 뒤 **"앱에서 열기" 딥링크**(`opscompanion://incidents/<id>`, 푸시와 같은 경로 — 매핑은 실기기 확인). 버린 안: 실제 상점을 세션 단위로 고장 내는 카오스 플래그(운영 코드에 고장 스위치가 더 큰 흠) · 대시보드 역방향 위젯(이력서 뒤) · 백엔드 테스트 API(프론트 소스맵 경로를 안 탄다).
- 범위: 웹 페이지 1개(`(admin)/admin/ops-app`) + 메뉴 항목 · 백엔드 폴러 필터 + 단위 1건 · 앱 `ReviewSummaryCard` 한 줄(`eas update`) · README/부록. **DB 0 · 비밀값 0 · 네이티브 0.**
- DoD: 운영 웹 데모 로그인 → 버튼 → 2분 내 앱 데모 목록에 코드 표시 · 첫 발생 시각 일치 · 분석이 그 페이지 소스를 읽음(머지·배포 후) · 채점 뒤 S4 "내 판정" · 푸시/Slack 없음.
