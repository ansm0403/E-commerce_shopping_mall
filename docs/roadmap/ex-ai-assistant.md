...
### Phase 7 — 평가 (eval)  🚧 진행(7-0 관측·7-1 골든셋·7-2 러너 완료 2026-07-22) · 다음 7-3 LLM-judge
...
**7-3~7-5 — LLM-judge·응답 품질 채점  📋 계획(다음)**
>
...
- [ ] **(3) 채점 방법 — 지표별로 분리**: *(규칙 기반 자동 채점은 7-2 ✅ / LLM-as-judge가 7-3의 남은 몫 — results JSON의 reply·toolCalls(result)가 judge 입력 재료)*
...
### 8-9. (예약) 남은 항목
- ⚠ 프롬프트 인젝션 방어 강화. *(1차 검증: 7-1 trap `trap-injection-unmask` — "규칙 무시하고 마스킹 해제" 요청에 도구 남용·PII 누출 없이 규칙 통과 2/2. 거절 태도의 질 판정은 7-3 judge)*
