...
**그리고 [`docs/learning/ops-companion/infra-story.md`](../learning/ops-companion/infra-story.md) 에
갱신 여지가 있는지 확인해 있으면 고쳐라.** 이 문서는 편 번호가 붙은 학습 노트와 달리 **살아 있는
문서**다 — Phase 종료 시점의 기록이 아니라 **현재 인프라의 지도**이므로, 인프라가 늘거나 연결이
바뀌면 그때그때 고쳐 쓴다(덧붙이기가 아니라 고쳐 쓰기다).

Phase 2 는 갱신 대상이 확실하다. 그 문서 **8장 "앞으로 바뀔 것"** 표에 Phase 2 몫으로 세 줄이
미리 적혀 있다:

| Phase 2 에서 생기는 변화 | 고칠 곳 |
|---|---|
| **소스맵 업로드** — EAS Build 가 빌드 끝에 Hermes 소스맵을 Sentry 로 올린다 → `expo.dev ↔ Sentry` 사이에 **선이 하나 새로 생긴다** | 2장(빌드 흐름) · 0-1 전체 지도 |
| **Release Health** — 앱이 릴리즈·세션 정보를 Sentry 로 보내고 백엔드가 crash-free 수치를 읽는다 | 4-6(Sentry 의 역할 A·B) |
| **생체 인증** — 기기 보안 칩 안에서만 일어난다. 서버·DB 변경 없음 | 3-3(인프라가 늘지 않는다는 점을 기록) |

그 밖에 새 자격증명(Sentry auth token 을 빌드 단계에서 쓰게 되는지 등)이나 비밀값이 하나라도
늘면 **5장 비밀값 지도**와 **6장 "무엇이 죽으면 무엇이 멈추나"** 도 함께 고친다. 새 용어는 본문
첫 등장 자리에서 풀고 **용어 사전**에도 넣는다. 끝으로 그 문서 맨 아래 **갱신 기록** 표에
날짜·커밋·바뀐 내용을 한 줄 남기고, README 부록 표의 "마지막 갱신" 도 맞춘다.
끝낸 항목은 8장 표에서 지우고 본문에 녹인다.
