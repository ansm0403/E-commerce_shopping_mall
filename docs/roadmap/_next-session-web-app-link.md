...
### C. 프론트 — 페이지 + 메뉴
- `frontend/src/app/(admin)/admin/app-test/page.tsx`(클라이언트 컴포넌트) + `AdminSidebar.tsx NAV_ITEMS` 에 `{ href: '/admin/app-test', label: '앱 연동 테스트' }`(감사 로그 아래).
- 페이지 구성(위에서 아래로):
  1. 한 단락: 이 페이지가 무엇인지(운영 앱이 실제 Sentry 와 연결돼 있음을 직접 확인).
  ...
  4. **앱에서 할 일** 4단계: 목록에서 `[방문자 테스트 XXXX]` 찾기 → 상세의 첫 발생 시각 비교 → AI 분석(새 분석은 시간당 6건, 방문자 합산 — 한도면 이미 분석된 다른 인시던트로) → 평가 탭에서 채점 → 분석 화면에서 "내 판정" 확인.
...
1. 운영 웹 → 데모 로그인 → 앱 연동 테스트 → 버튼 → **2분 이내** 앱(데모 계정) 목록에 `[방문자 테스트 XXXX]` 표시, 상세의 첫 발생 시각 = 버튼 시각.
...
목표: 웹 관리자 페이지에 "앱 연동 테스트" 페이지를 만들어, 방문자가 버튼 한 번으로 실제 에러를 내고 2분 안에 앱 인시던트 목록에서 자기 코드가 붙은 인시던트를 찾아 분석·채점한 뒤 "내 판정"까지 보게 한다.
